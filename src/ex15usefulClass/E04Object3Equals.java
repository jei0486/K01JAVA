...
class MyClass {
	...
	/*
	 MyClass형의 객체의 비교를 위해 오버라이딩 한 메소드
	 객체가 갖고있는 멤버변수의 비교를 통해 동일여부를 판단한다.
	 */
	@Override
	public boolean equals(Object obj) {
		...
		if(obj instanceof MyClass) {
			...
			if(mc.data==this.data) {
				...
				return false;//값이 다를때 false
			}
		}
		else {
			/*
			 매개변수로 전달된 객체가 MyClass의 인스턴스가 아니라면 비교의 대상이
			 될 수 없으므로 false반환함
			 */
			return false;
		}
...
public class E04Object3Equals {
	...
	public static void main(String[] args) {
		...
		//print문 자체에 toString() 메소드 기능이 포함돼있으므로 toString()이 없어도
		//결과는 동일함
		...
		/*
		 아래 toString()메소드는 굳이 호출하지않아도 print()문이 실행될때
		 자동으로 호출하므로, 아래 출력결과는 동일함
		 */
