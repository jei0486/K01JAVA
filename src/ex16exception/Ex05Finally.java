...
/*
 FINALLY절
 	: 예외발생여부와 상관없이 try문으로 진입했을떄
 	반드시 실행해야하는 코드가 있는경우 기술하는 블럭이다.
 	즉 try문으로 진입되지않는다면 finally절은 실행되지않는다.
 	
 	1)try~catch : 예외를 직접처리할 때 
...
public class Ex05Finally {
	...
	static void tryCatchFinally() {
		...
		try {
			...
			//try구문에서 return을 만나더라도 finally는 실행된다.
			return;
			...
			//프로그램 자체가 종료된다. System 클래스
			/*
			 try문안에서 exit(0)를 실행하게되면 프로그램자체가 종료되므로
			 finally 구문은 실행되지않는다.
			 */
			System.exit(0);
		}
