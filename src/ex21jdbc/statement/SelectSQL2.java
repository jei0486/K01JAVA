...
public class SelectSQL2 extends ConnectDB {
	...
	/*
	 ResultSet 클래스
	 	...
	 	결과롤 반환된 레코드의 처음부터 마지막까지 next()메소드를 통해
	 	확인후 반복하면서 추출하게 된다.
	...
	void execute() {
		try {
			...
			while (rs.next()) {
				String id = rs.getString(1);//인덱스로 추출
